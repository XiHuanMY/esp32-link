...
bool INA226_WE::init(){
    ...
    //     return 0;
    // }
    printf("ina226 id 0x%x\r\n",readRegister(INA226_MAN_ID_REG));


    
    reset_INA226();
    setAverage(AVERAGE_1);printf("555\r\n");
    setMeasureMode(CONTINUOUS); printf("555\r\n");
    setCurrentRange(MA_800);printf("555\r\n");
    convAlert = false;
    limitAlert = false;
    corrFactor = 1.0;
    printf("ina226 ok\r\n");
...
void INA226_WE::writeRegister(uint8_t reg, uint16_t val){
    ...
    data[2] = val & 255;
    data[1] = val >> 8;
    i2c_master_write_to_device(I2C_MASTER_NUM, 0x40, data, 3, I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
    
}
  
uint16_t INA226_WE::readRegister(uint8_t reg){
  uint8_t data[2];
  i2c_master_write_read_device(I2C_MASTER_NUM, 0x40, &reg, 1, data, 2, I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
