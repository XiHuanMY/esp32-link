...
void ColorWheel::Update(lv_color16_t *color)
{
    //uint16_t value = (uint16_t)color;
    
    uint8_t r = color->red; // Lấy 5 bit đầu tiên
    uint8_t g = color->green; // Lấy 6 bit tiếp theo
    uint8_t b = color->blue;         // Lấy 5 bit cuối cùng
    Model.Update(r, g, b);
}
...
void ColorWheel::onEvent(lv_event_t *event)
{
    ...
    if (obj == instance->View.ui.color.cont)
    {
        ...
        {
            lv_color16_t color_val ;//= 0xff0000;//lv_colorwheel_get_rgb(instance->View.ui.color.cont);
