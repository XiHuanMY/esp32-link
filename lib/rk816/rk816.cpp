...
{
	return (int)lvgl_i2c_write(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &val, 1);
}
...
int rk816_reg_read(uint8_t reg, uint8_t *val)
{
	...
	// *val = buf;
	// return 1;
	return (int)lvgl_i2c_read(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, val, 1);
}

uint8_t rk816_bat_read(uint8_t reg)
{
	uint8_t val=0;
	lvgl_i2c_read(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &val, 1);
...
{
	return (int)lvgl_i2c_read(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, val, size);
}

int rk816_reg_write_data(uint8_t reg, uint8_t *val, uint16_t size)
{
	return (int)lvgl_i2c_write(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, val, size);
}
...
void rk816_init_power(void)
{
	...
	printf("rk816 id  0x%x version  0x%x\r\n",  data[0] << 4 | ((data[1] >> 4) & 0xf), data[1] & 0xf);
