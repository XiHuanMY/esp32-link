...
{
	return (int)i2c_manager_write(I2C_MASTER_NUM,RK816_DEFAULT_I2C_ADDRESS, reg, val);
}

int rk816_reg_read(uint8_t reg, uint8_t *val)
{
	return (int)i2c_manager_read(I2C_MASTER_NUM,RK816_DEFAULT_I2C_ADDRESS, reg, val, 1);
}

uint8_t rk816_bat_read(uint8_t reg)
{
	uint8_t val=0;
	i2c_manager_read(I2C_MASTER_NUM,RK816_DEFAULT_I2C_ADDRESS, reg, &val, 1);
...
{
	return (int)i2c_manager_read(I2C_MASTER_NUM,RK816_DEFAULT_I2C_ADDRESS, reg, val, size);
}
...
void rk816_init_power(void)
{
	// read version
	uint8_t data[2];
	rk816_reg_read(RK816_CHIP_NAME_REG, &data[0]);
